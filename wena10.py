...
def update_our_score(game: Game, bot_state: DomainExpansionBotState):
    """Update our score tracking"""
    # The score is stored in the state's points attribute
    bot_state.our_score = game.state.points
...
def handle_place_tile(
    ...
    update_our_score(game, bot_state)
    ...
    # 2. MONASTERY PRIORITY (highest guaranteed points)
    for tile_hand_index, tile_in_hand in enumerate(game.state.my_tiles):
        if hasattr(tile_in_hand, "modifiers") and "MONESTARY" in [m.name for m in tile_in_hand.modifiers]:
            # Find placement near existing meeples
            expansion_pos = find_expansion_opportunities(
                game, bot_state, tile_in_hand, StructureType.MONASTARY
            )
            if expansion_pos:
                x, y = expansion_pos
    ...
    # 3. COOPERATIVE BUILDING (Shared Joy Principle)
    for tile_hand_index, tile_in_hand in enumerate(game.state.my_tiles):
        for structure_type in [StructureType.CITY, StructureType.ROAD]:
            # Look for large features to join
            expansion_pos = find_expansion_opportunities(
                game, bot_state, tile_in_hand, structure_type
            )
            if expansion_pos:
                x, y = expansion_pos
                bot_state.last_tile = tile_in_hand
...
def handle_place_meeple(
    ...
    # 2. QUICK COMPLETION PRIORITY (small structures we can complete soon)
    for edge, structure in structures:
        ...
        if game.state._check_completed_component(recent_tile, edge):
            continue
            
        # Use simple heuristics for quick completion
    ...
    # 3. STRUCTURE STEALING PRIORITY (for large unfinished structures)
    for edge, structure in structures:
        structure_type = recent_tile.internal_edges.get(edge)
        ...
        # Get existing claims on this structure
        claims = game.state._get_claims(recent_tile, edge)
        if not claims:  # Skip unclaimed structures (handled elsewhere)
            continue
            
        # Analyze claim distribution
        claims_by_player = defaultdict(int)
        for claim in claims:
            claims_by_player[claim.player] += 1

        our_claims = claims_by_player.get(game.state.our_player_index, 0)
        max_opponent_claims = max(
            (count for player, count in claims_by_player.items() 
             if player != game.state.our_player_index),
            default=0
        )
        
        # Get structure size
        structure_size = game.state.get_component_size(recent_tile, edge)
        
        # Stealing conditions:
        # - Structure is large (cities >3 tiles, roads >4 tiles)
        # - We have available meeples
        # - Adding our meeple would tie or take the lead
        if (
            ((structure_type == StructureType.CITY and structure_size > 7) or 
             (structure_type == StructureType.ROAD and structure_size > 8)) and
            (our_claims + 1) >= max_opponent_claims
        ):
            bot_state.meeples_placed += 1
            meeple_info = MeepleInfo(
                recent_tile, 
    ...
    # 4. GENERAL PLACEMENT (any available uncompleted feature)
    for edge, structure in structures:
        structure_type = recent_tile.internal_edges.get(edge)
        if structure_type not in [StructureType.CITY, StructureType.ROAD]:
            continue
        ...
        # Place only if no existing claims
        if not game.state._get_claims(recent_tile, edge):
            bot_state.meeples_placed += 1
            meeple_info = MeepleInfo(
                recent_tile, 
